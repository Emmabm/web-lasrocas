...
const MainCourseSelectionMenu3: React.FC = () => {
  ...
  const continuar = async () => {
    ...
    const { error } = await supabase.rpc('upsert_menu3_formularios', {
      p_event_id: eventId,
      p_paso: 'plato-principal-menu3',
      p_datos: { tipo: 'parrillada' },
    });
  ...
  const volver = () => {
    navigate(`/catering/menu3/recepcion?token=${token}`);
  };
    ...
    <div className="bg-white flex justify-center pt-10 pb-6 px-6">
      <div className="max-w-5xl w-full bg-white rounded-2xl shadow-lg p-8">
        <h1 className="text-3xl sm:text-4xl font-serif font-bold text-center text-gray-800 mb-4 sm:mb-6">
          Plato Principal — Menú 3 (Parrillada)
        </h1>
        <p className="text-center text-gray-600 text-lg mb-4">
          El plato principal incluye una <span className="text-[#FF6B35] font-semibold">parrillada completa</span>:
        </p>
        <div className="bg-orange-50 border border-[#FF6B35] rounded-lg p-6 mb-8 text-center">
          <p className="text-gray-700 text-base">
            Distintos cortes y variedades de carnes cocinadas a la parrilla, servido en la mesa por mozos que pasan constantemente y acompañado por variedades de ensaladas.
          </p>
        </div>
...
export default MainCourseSelectionMenu3;